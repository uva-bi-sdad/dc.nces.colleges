# 4-year, 2-year Colleges and Trade Schools Access Scores in VA at the Health District Level
# packages
library(tigris)
library(sf)
library(dplyr)
...
inds_4year <- st_within(geopts_4year$geometry, counties$geometry, sparse=T)
inds_2year <- st_within(geopts_2year$geometry, counties$geometry, sparse=T)
inds_less2 <- st_within(geopts_less2$geometry, counties$geometry, sparse=T)
...
for (i in inds_4year){
  if (identical(counties$NAME[i],character(0))){
    ...
    ct_list_4year <- append(ct_list_4year, counties$GEOID[i])}
}
for (i in inds_2year){
  if (identical(counties$NAME[i],character(0))){
    ct_list_2year<- append(ct_list_2year, NA)}
  else{
    ct_list_2year <- append(ct_list_2year, counties$GEOID[i])}
}
for (i in inds_less2){
  if (identical(counties$NAME[i],character(0))){
    ct_list_less2<- append(ct_list_less2, NA)}
  else{
    ct_list_less2 <- append(ct_list_less2, counties$GEOID[i])}
}
...
supply_4year$GEOID <- as.character(supply_4year$GEOID)
supply_2year$GEOID <- as.character(supply_2year$GEOID)
supply_less2$GEOID <- as.character(supply_less2$GEOID)

# add health districts column to geometries
supply_4year <- left_join(supply_4year, health_district[, c("geoid_county", "geoid")],
                        by = c("GEOID" = "geoid_county"))
supply_2year <- left_join(supply_2year, health_district[, c("geoid_county", "geoid")],
                          by = c("GEOID" = "geoid_county"))
supply_less2 <- left_join(supply_less2, health_district[, c("geoid_county", "geoid")],
                          by = c("GEOID" = "geoid_county"))
#####################
# DEMAND
#####################
...
# Reproject
va.ct.utm <- st_transform(va.ct, crs = "+proj=longlat +datum=WGS84")

# add health districts column to geometries
demand <- left_join(va.ct.utm, health_district[, c("geoid", "geoid_county", "region_name")], by = c("GEOID" = "geoid_county"))

demand <- demand  %>%
  group_by(geoid, region_name) %>%
  summarise(geometry = sf::st_union(geometry),
            pop_over15 = sum(pop_over15)) %>%
  ungroup()
...
demand <- st_sf(geoid = demand$geoid,
                ...
                pop_over15 = demand$pop_over15, geometry = demand$geometry) %>%
  mutate(centroid = st_centroid(geometry))

demand <- demand %>% extract(centroid, c('lon', 'lat'), '\\((.*), (.*)\\)', convert = TRUE)
...
out_df_4year <- left_join(out_df_4year, demand[,c("geoid", "region_name")], by=c("geoid"))
out_df_2year <- left_join(out_df_2year, demand[,c("geoid", "region_name")], by=c("geoid"))
out_df_less2 <- left_join(out_df_less2, demand[,c("geoid", "region_name")], by=c("geoid"))

out_df_4year <- out_df_4year %>% select(-geoid)
out_df_2year <- out_df_2year %>% select(-geoid)
out_df_less2 <- out_df_less2 %>% select(-geoid)

out_df_4year <- left_join(out_df_4year, health_dist, by="region_name")
out_df_2year <- left_join(out_df_2year, health_dist, by="region_name")
out_df_less2 <- left_join(out_df_less2, health_dist, by="region_name")
...
dbWriteTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_4year_colleges"),
             df_long_4year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_2year_colleges"),
             df_long_2year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_trade_schools"),
             df_long_less2,  row.names = F)

dbRemoveTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_4year_colleges"))
dbRemoveTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_2year_colleges"))
dbRemoveTable(con, c("dc_education_training", "va_hd_osrm_2021_drive_times_nearest_trade_schools"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_hd_osrm_2021_drive_times_nearest_4year_colleges
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_hd_osrm_2021_drive_times_nearest_2year_colleges
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_hd_osrm_2021_drive_times_nearest_trade_schools
                    OWNER TO data_commons")
...
df3_4year <- df3_4year %>% select(c("region_name", "e2sfca", "fca2s",
                                    "fca3s", "capacity", "norm_fca3s",
                                    "norm_fca2s", "norm_efca2s"))

df3_2year <- df3_2year %>% select(c("region_name", "e2sfca", "fca2s",
                                    "fca3s", "capacity", "norm_fca3s",
                                    "norm_fca2s", "norm_efca2s"))

df3_less2 <- df3_less2 %>% select(c("region_name", "e2sfca", "fca2s",
                                    "fca3s", "capacity", "norm_fca3s",
                                    "norm_fca2s", "norm_efca2s"))

# add health distrs geoids
out_df_4year <- left_join(df3_4year, health_dist, by=c("region_name"))
out_df_2year <- left_join(df3_2year, health_dist, by=c("region_name"))
out_df_less2 <- left_join(df3_less2, health_dist, by=c("region_name"))
