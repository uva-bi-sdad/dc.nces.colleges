# 4-year, 2-year Colleges and Trade Schools Access Scores in VA at the block group level
# packages
library(tigris)
library(sf)
library(dplyr)
...
# VA block groups geoids
blocks <- st_as_sf(block_groups(state="VA"))
...
inds_4year <- st_within(geopts_4year$geometry, blocks$geometry, sparse=T)
inds_2year <- st_within(geopts_2year$geometry, blocks$geometry, sparse=T)
inds_less2 <- st_within(geopts_less2$geometry, blocks$geometry, sparse=T)
...
for (i in inds_4year){
  if (identical(blocks$NAME[i],character(0))){
    ...
    ct_list_4year <- append(ct_list_4year, blocks$GEOID[i])}
}
for (i in inds_2year){
  if (identical(blocks$NAME[i],character(0))){
    ct_list_2year<- append(ct_list_2year, NA)}
  else{
    ct_list_2year <- append(ct_list_2year, blocks$GEOID[i])}
}
for (i in inds_less2){
  if (identical(blocks$NAME[i],character(0))){
    ct_list_less2<- append(ct_list_less2, NA)}
  else{
    ct_list_less2 <- append(ct_list_less2, blocks$GEOID[i])}
}
...
va.ct <- get_acs(geography = "block group",
                 year = 2019,
...
block_names <- geo_names %>% filter(region_type=="block group")

out_df_4year <- left_join(out_df_4year, block_names, by=c("geoid"))
out_df_2year <- left_join(out_df_2year, block_names, by=c("geoid"))
out_df_less2 <- left_join(out_df_less2, block_names, by=c("geoid"))
...
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_4year_colleges"),
             df_long_4year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_2year_colleges"),
             df_long_2year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_trade_schools"),
             df_long_less2,  row.names = F)

# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_4year_colleges"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_2year_colleges"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2021_drive_times_nearest_trade_schools"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2021_drive_times_nearest_4year_colleges
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2021_drive_times_nearest_2year_colleges
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2021_drive_times_nearest_trade_schools
                    OWNER TO data_commons")
...
# add region names
out_df_4year <- left_join(df3_4year, block_names, by=c("GEOID"="geoid"))
out_df_2year <- left_join(df3_2year, block_names, by=c("GEOID"="geoid"))
out_df_less2 <- left_join(df3_less2, block_names, by=c("GEOID"="geoid"))
...
dbWriteTable(con, c("dc_education_training", "va_bg_sdad_2019_4year_colleges_access_scores"),
             out_df_long_4year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_sdad_2019_2year_colleges_access_scores"),
             out_df_long_2year,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_sdad_2019_trade_schools_access_scores"),
             out_df_long_less2,  row.names = F)

dbRemoveTable(con, c("dc_education_training", "va_bg_sdad_2019_4year_colleges_access_scores"))
dbRemoveTable(con, c("dc_education_training", "va_bg_sdad_2019_2year_colleges_access_scores"))
dbRemoveTable(con, c("dc_education_training", "va_bg_sdad_2019_trade_schools_access_scores"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_sdad_2019_4year_colleges_access_scores
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_sdad_2019_2year_colleges_access_scores
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_sdad_2019_trade_schools_access_scores
                    OWNER TO data_commons")
